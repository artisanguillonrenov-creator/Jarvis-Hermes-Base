    ...
    with _core._lock:
        ...
        for key, scopes in _core._server_tool_scopes.items():
            ...
            if session is not None:
                # A live session clears any pending transient mark: a future None starts a fresh grace.
                _core._server_session_none_at.pop(key, None)
            elif server is None:
                stale.append(key)
            elif (config is None or not _server_enabled(config)
                  or not _same_server_route(server, config, cross_profile=cross_profile)):
                # Config-side reasons still evict immediately — only the ``session is None``
                # branch is deferred.
                _core._server_session_none_at.pop(key, None)
                stale.append(key)
            else:
                # ``session is None`` on a known live connection is ambiguous — a transient HTTP
                # keepalive gap looks identical to a dead server. Defer the eviction until the
                # observation persists past ``_SESSION_NONE_GRACE_SECONDS``; a recovery within that
                # window clears the mark and the tools stay registered (#109798).
