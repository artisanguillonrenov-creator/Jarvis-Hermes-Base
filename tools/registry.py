...
#
# Prose-heavy tool descriptions are a per-request tax: the biggest core tools
# alone carry ~9 KB of description + parameter prose re-sent on every API call.
# A tool may register `compact_description` and `compact_parameter_descriptions`
# alongside its schema; when the profile is on, get_definitions() swaps that
# prose in. Structure — parameter names, types, enums, required lists — is never
# touched, so tool-calling behaviour is unaffected.
#
# Prompt caching: the verdict is read once per get_definitions() pass and tool
# definitions are frozen for the life of a session, so flipping the config never
# rewrites an in-flight conversation's schemas.
# ---------------------------------------------------------------------------
...
class ToolEntry:
    ...
    # Both are prose-only: structure is never rewritten, and a tool that supplies
    # neither is served unchanged under either profile.
