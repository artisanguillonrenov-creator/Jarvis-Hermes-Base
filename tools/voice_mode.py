...
def stop_playback() -> None:
    ...
    if proc and proc.poll() is None:
        ...
        # We deliberately stopped this player: clear the shared pidfile so a
        # sibling Hermes process doesn't SIGTERM a recycled PID.
        with suppress(Exception):
...
# exits — auto-continuing would immediately re-fight the newer playback.
# ---------------------------------------------------------------------------
...
def _system_player_candidates(file_path: str) -> List[List[str]]:
    ...
    if system == "Linux":
        # aplay cannot decode MP3/OGG: it plays the raw bytes as PCM (white
        # noise) and exits 0, which looks like success (#109029). Only offer
        # it for container formats ALSA actually decodes.
        if file_path.lower().endswith((".wav", ".wave", ".au", ".voc")):
            players.append(["aplay", "-q", file_path])
    return players


def _run_system_player(cmd: List[str]) -> Optional[bool]:
    """Run one player to completion (interruptible via stop_playback).

    Returns ``True`` on success, ``False`` on player failure (caller should
    try the next candidate), and ``None`` when playback was deliberately
    interrupted — superseded by a newer request or stopped — meaning the
    caller must NOT fall through to another player (re-spawning would layer
    a second stream over whoever superseded us).

    Global playback arbitration: a NEW playback request always interrupts the
    previous one BEFORE starting. Concurrent sessions/windows/profile backends
    hitting the same process can otherwise layer multiple afplay streams on
    top of each other (多会话同时说话叠音). Latest request wins — barge-in
    semantics, same as stop_playback() but atomic with the spawn so there is
    no window where two players coexist. wait() stays OUTSIDE the lock so
    stop_playback() can still kill us mid-play.
    """
    global _active_playback
    proc = None
    try:
        ...
        with _playback_lock:
            ...
            if prev is not None and prev.poll() is None:
                with suppress(Exception):
                    prev.terminate()
                    # Bounded wait for the old player to actually exit so the
                    # new one never overlaps it even briefly (SIGTERM delivery
                    # is async).
                    with suppress(Exception):
                        prev.wait(timeout=0.5)
            # Cross-process: kill a live player owned by another Hermes
            # process (serve vs gateway vs CLI) before we start, then record
            # ours so the next process can do the same to us.
        ...
        with _playback_lock:
            ...
            if was_current:
                ...
                _clear_playback_pid(proc.pid)
        if not was_current:
            # We were superseded (a newer playback request barge-in'd us, or
            # stop_playback() ran). Abort the player loop entirely.
            logger.debug("playback superseded before completion: %s", cmd[0])
            return None
        if rc < 0:
            # Terminated by a signal. Discriminate the source for
            # observability (review #85793 by arnaudlb): -15 is our own
            # arbitration/stop path; -9 and friends mean an external hard
            # kill (OOM killer). Neither falls through — respawning another
            # player under memory pressure is the wrong recovery, and
            # falling through on a cross-process stop is the bug this guard
            # exists to prevent (re-spawning the stream another Hermes
            # process deliberately interrupted).
            if rc == -signal.SIGTERM:
                logger.debug("playback interrupted by SIGTERM (arbitration/stop): %s", cmd[0])
            else:
                logger.warning("player terminated by signal %d (possible OOM kill): %s", -rc, cmd[0])
            return None
        if rc == 0:
            return True
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    for cmd in _system_player_candidates(file_path):
        if not shutil.which(cmd[0]):
            continue
        outcome = _run_system_player(cmd)
        if outcome is True:
            return True
        if outcome is None:
            # Deliberately interrupted (newer playback / stop / cross-process
            # arbitration): do NOT try the next player — that would re-spawn
            # the stream we were interrupted on and re-layer audio.
            return False
