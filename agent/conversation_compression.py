...
def _fold_todo_snapshot(agent: Any, compressed: list) -> None:
    ...
    # With a fresh snapshot, the injector must observe stale rows itself so it
    # can distinguish the intent-ordering bug case from the release's normal
    # tail-only behavior. Retain the legacy cleanup pass only when there is no
    # replacement snapshot to inject (for example, all todos were retired).
    if _todo_store_is_authoritative and not todo_snapshot:
