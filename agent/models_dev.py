...
def _provider_model_capabilities(provider: str, model: str) -> Dict[str, Any]:
    """Exact-model declaration from the existing provider plugin registry."""
...
def _apply_overrides(provider: str, model: str, entry: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Catalog/builtin metadata, patched by plugin then explicit user overrides.

    Fill-gap defaults apply only when none of those sources knows the model.
    """
    ...
    if declared:
        base = _merge_catalog_entry_with_override(
            base if base is not None else _UNKNOWN_MODEL_BASE, declared)
