...
CODING_AGENT_GUIDANCE = (
    ...
    "\n"
    "Respect the user's repo: do not commit or push outside a user-approved "
    "workflow. Project instructions may authorize a local checkpoint commit "
    "for a complete, verified deliverable. Treat push separately according to "
    "destination and downstream effects. Never force-push or rewrite history "
    "without explicit authorization for that action. Never read, print, or "
    "commit secrets — leave `.env` and credential files alone unless the user "
    "explicitly asks. The Workspace "
