...
def build_kanban_stop_nudge(
    *,
    ...
    return (
        ...
        "1. Call `kanban_complete(summary=..., artifacts=[...])` immediately if "
        "the work is already satisfied, including as a coordinator/orchestrator, "
        "OR call `kanban_block(reason=...)` immediately if you are blocked.\n"
        "2. If any deliverable remains and you are not blocked, finish the actual "
        "remaining deliverable (write the required file(s) now) before calling "
        "`kanban_complete`.\n\n"
