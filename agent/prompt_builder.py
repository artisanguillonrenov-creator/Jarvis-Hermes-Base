...
def _render_skills_index(
    ...
    return (
        "## Skills\n"
        "Load any skill below that is even partially relevant with skill_view(name) and follow it — "
        "skills carry endpoints, pitfalls, and workflows that beat general-purpose approaches, even for "
        f"tasks you could handle with basic tools like {_basic_tools}. "
        "Fix broken skills with skill_manage(action='patch'); after hard tasks, offer to save "
        "the workflow as a skill.\n"
        "\n"
        "<available_skills>\n"
        + "\n".join(index_lines) + "\n"
        "</available_skills>\n\n"
        "Skip loading only when genuinely none apply."
        + hidden_note
    )
