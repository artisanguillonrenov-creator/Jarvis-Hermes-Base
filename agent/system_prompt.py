...
def _skills_prompt(agent: Any) -> str:
    ...
    # Focus mode demotes non-coding categories to names-only. Independently,
    # agent.skills_catalog_mode can demote the catalog on any surface
    # (chat/messaging defaults to `compact`). Both sets resolve once from
    # session-fixed inputs and are unioned into a single compact_categories
    # argument, so the render stays a single cache-safe entry.
    ...
    try:
        ...
        try:
            ...
            _cfg = None
        _catalog_cats = resolve_skills_catalog_compaction(platform=agent.platform, config=_cfg)
    except Exception:
        _compact_cats = frozenset()
        _catalog_cats = frozenset()
    ...
    # frozenset, so `_resolved_cats or None` would silently coerce it to None.
    # build_skills_system_prompt identity-checks the sentinel and treats an
    # empty plain frozenset the same as None, so forwarding verbatim is correct.
    return _pb.build_skills_system_prompt(available_tools=agent.valid_tool_names, available_toolsets=avail_toolsets,
                                         compact_categories=_resolved_cats, skills_dir_override=_agent_skills_dir(agent))
