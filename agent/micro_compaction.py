...
def _is_summary_marker(entry: Any) -> bool:
    return isinstance(entry, dict) and bool(
        entry.get(_cc().COMPRESSED_SUMMARY_METADATA_KEY)
    )
...
class MicroCompactionMixin:
    ...
    def _resolve_compact_cursor(
        self, messages: List[Dict[str, Any]], head_end: int, tail_start: int
    ) -> int:
        ...
        summaries = (
            ...
            if self._is_context_summary_message(messages[i])
        )
        ...
        if last >= head_end:
            ...
            recovered = (
                ""
                if self._micro_compact_rolling_summary.strip()
                else (self._rolling_summary_from_marker(messages[last].get("content")))
            )
            if recovered:
                ...
                logger.info(
                    "Micro-compaction: recovered rolling summary from transcript (%d chars)",
                    len(recovered),
                )
    ...
    def _find_one_exchange(
        ...
        def _turn_row(idx: int, roles: tuple) -> bool:
            return messages[idx].get(
                "role"
            ) in roles and not self._is_context_summary_message(messages[idx])
        ...
        if not isinstance(boundary, dict) or boundary.get("role") in (
            "assistant",
            "tool",
        ):
            return None
    ...
    def _build_micro_summary_prompt(
        self, existing_summary: str, exchange_text: str
    ) -> List[Dict[str, str]]:
        """Build the prompt messages for a single-exchange micro-summary."""
        summary_block = (
            existing_summary
            if existing_summary.strip()
            else "(No previous summary yet.)"
        )
        user_prompt = (
    ...
    def _needs_defrag(self) -> bool:
        ...
        return (
            estimate_tokens_rough(self._micro_compact_rolling_summary)
            >= self._micro_compact_defrag_threshold_tokens
        )
    ...
    def _micro_compact(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def _telemetry(
            outcome: str, result: List[Dict[str, Any]], **extra: Any
        ) -> None:
        ...
        if self._needs_defrag():
            ...
            _telemetry(
                outcome, messages, tokens_after=estimate_messages_tokens_rough(messages)
            )
            return messages
    ...
    def _record_micro_failure(self, exchange_start: int, exchange_end: int) -> str:
        ...
        self._micro_compact_consecutive_failures = (
            self._micro_compact_consecutive_failures + 1 if same_cursor else 1
        )
        self._micro_compact_last_failure_cursor = exchange_start
        if (
            self._micro_compact_consecutive_failures
            < _cc()._MICRO_COMPACT_MAX_CONSECUTIVE_FAILURES
        ):
    ...
    def _next_exchange(
        ...
        return (
            ...
            else self._find_one_exchange(messages, cursor, compress_end)
        )
    ...
    def _rolling_summary_from_marker(content: Any) -> str:
        ...
        body = (
            content[idx + len(cc.HISTORICAL_TASK_HEADING) :] if idx != -1 else content
        )
    ...
    def _emit_micro_compaction_telemetry(
        ...
        *,
        ...
        try:
            ...
            occupancy = (
                round(tokens_after / threshold * 100, 1) if has_occupancy else None
            )
            payload = {
    ...
    def _splice_micro_compact_result(
        ...
        stale = (
            [i for i, m in enumerate(result) if _is_micro_marker(m)][:-1]
            if supersede
            else []
        )
        if stale:
            result = self._merge_adjacent_user_turns([
                m for i, m in enumerate(result) if i not in stale
            ])
