...
_MEMORY_REVIEW_PROMPT = (
    ...
    "Calibration (applies to every save):\n"
    "  • File what the USER said, not what you fetched or proposed. Web/connector "
    "research is re-queryable and stays out; your recommendations stay out until "
    "the user confirms one — then file the confirmation.\n"
    "  • Your own earlier suggestions are never the user's decisions. Before "
    "saving 'user wants X', a user message must actually state it; silence is "
    "not agreement.\n"
    "  • A single passing mention of a taste or pastime is not yet memory "
    "material — file when it recurs or the user dwells on it. Stable world "
    "facts (people, roles, relationships, ongoing projects) file on first "
    "mention.\n"
    "  • Match the level of engagement: a brief 'sounds good' confirms the "
    "shape of what you said, not each detail inside it.\n"
    "  • Prefer durable phrasing over precise figures that go stale.\n"
    "  • Already filed means already remembered: restate nothing; only file "
    "what changes the store.\n"
    "  • Horizon test: would this still be true and worth reading a month "
    "from now, in a conversation about something else? Task-state that dies "
    "with the task stays out.\n\n"
...
_SKILL_REVIEW_PROMPT = (
    ...
    "or (3). Every new or patched skill's description MUST be a trigger contract: one sentence, "
    "use-when FIRST ('Use when X. Does Y.'), under 60 chars so the index doesn't truncate it, "
    "with a negative trigger when a sibling skill is easily confused.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "preferences with the memory tool. Calibration: file what the USER said, not what you fetched "
    "or proposed — research is re-queryable, your recommendations are yours until the user "
    "confirms one (then file the confirmation). Your own earlier suggestions are never the "
    "user's decisions; silence is not agreement. A single passing mention of a taste is not yet "
    "memory material — file when it recurs or the user dwells on it; stable world facts (people, "
    "roles, relationships, ongoing projects) file on first mention. Already filed means already "
    "remembered — restate nothing. Horizon test: would this still be worth reading a month "
    "from now, in a conversation about something else?\n\n"
    ...
    "If the name only fits today's task, fall back to (1), (2), or (3). Every new or patched "
    "skill's description MUST be a trigger contract: one sentence, use-when FIRST ('Use when X. "
    "Does Y.'), under 60 chars so the index doesn't truncate it, with a negative trigger when a "
    "sibling skill is easily confused.\n\n"
