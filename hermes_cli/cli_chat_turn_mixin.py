...
class CLIChatTurnMixin:
    ...
    def chat(self, message, images: list = None, voice_input: bool = False) -> Optional[str]:
        ...
        if not self._ensure_runtime_credentials():
            ...
            self._last_turn_result = {"failed": True, "failure_reason": "credentials"}
            return None
        ...
        if not self._init_agent(model_override=turn_route["model"], runtime_override=turn_route["runtime"],
            ...
            self._last_turn_result = {"failed": True, "failure_reason": "init"}
            return None
        agent = self.agent
        if agent is None:
            self._last_turn_failed = True
        ...
        try:
            ...
            self._last_turn_result = {"failed": True, "failure_reason": "exception"}
            return None
        finally:
