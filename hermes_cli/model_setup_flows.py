...
def _aigateway_models(pconfig, curated, api_key, base_url):
    """AIgateway: live first, then models.dev, then curated."""
    from hermes_cli.models import fetch_api_models
    live_models = fetch_api_models(api_key, base_url)
    if live_models:
        _report_live_models(live_models, f"{pconfig.name} API")
        return live_models
    model_list = _models_dev_merged("aigateway", curated)
    if model_list:
