...
def run_doctor(args):
    ...
    # Incompatible with --fix / --ack to preserve the existing surface.
    # Emits text, --json, or --compact and returns before ordinary doctor.
    # ------------------------------------------------------------------
    if getattr(args, 'upstream', False):
        if should_fix or getattr(args, 'ack', None):
            print(color(
