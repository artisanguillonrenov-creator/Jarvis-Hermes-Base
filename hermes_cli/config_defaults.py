...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        "max_verify_nudges": 3,
        # Opt-in: consult `pre_verify` hooks on turns that edited NO files too, so a policy
        # hook can demand verification for an answer-only turn (stated numbers, versions,
        # paths). False keeps the gate edit-only — zero hook calls, zero added cost (#109815).
        "pre_verify_without_edits": False,
