...
def _write_profile_model(
    ...
    provider: str,
    model: str,
    validate_in: Optional[Path] = None,
) -> None:
    """Validate a model in its source catalog and write it into ``profile_dir``.

    A config-map provider also carries its allowlisted, non-secret routing definition from the
    source profile. ``validate_in`` is the home whose catalog offered the pick; profile creation
    uses the dashboard home because the new profile has no credentials or provider map yet.
    """
    from hermes_cli.config import load_config, save_config

    source_home = validate_in or profile_dir
    with _hermes_home_scope(source_home):
