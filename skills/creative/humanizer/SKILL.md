...
Load this skill whenever the user asks to "humanize", "de-AI", "de-slop", or "un-ChatGPT" a piece of text; to rewrite something so it does not sound like an LLM wrote it; to edit a draft (blog post, essay, PR description, docs, memo, email, tweet, resume bullet) to sound more natural; to match their voice in writing they are producing; or to review text for AI tells before publishing.
...
**File mode.** When the user names a file, run the full process but write only the final text to the file. Use `read_file` to load it, then `patch` for targeted per-section edits (cleaner for a markdown doc in a repo) or `write_file` for a full rewrite. Change prose only. Keep code blocks, inline code, commands, paths, YAML metadata, data, and link targets unchanged. Then give the user a short summary.
...
**Marketing and blog cliches (same tell, different register):** at the end of the day, when it comes to, in a world where, moving forward, circle back, deep dive, game-changer, double down, take a step back, on the same page, make no mistake, it turns out, let me be clear, navigate (for challenges), lean into, unpack (before analysis), straightforward (to describe anything)

**Problem:** Models use these words far more often than people do, especially in groups. These cliches appear far more frequently in post-2023 text and often co-occur. This is the only vocabulary list in the skill. A formal word outside it is not a tell by itself.
**Before:**
...
Original author: Siqi Chen ([@blader](https://github.com/blader)). Original repo: https://github.com/blader/humanizer (version 3.0.0). Ported to Hermes Agent with Hermes-native tool references (`read_file`, `patch`, `write_file`), a "when to load" trigger list, and a marketing and blog cliche list added under section C. The extra patterns the 2.9.x Hermes port had added (writing about the previous version, a heading repeated below itself, formulaic sayings, fake-candid openings, staged run-ups, rejecting fake alternatives) were absorbed upstream in 3.0.0 and are no longer Hermes-specific. The skill's own instructional prose follows its own guidance: the narration carries no em dashes or negative parallelism, so the skill models the writing it asks for. Original MIT license preserved in the `LICENSE` file alongside this `SKILL.md`.
