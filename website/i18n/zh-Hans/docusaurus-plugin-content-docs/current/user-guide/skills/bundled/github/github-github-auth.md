...
uv run python3 "${HERMES_HOME:-$HOME/.hermes}/skills/software-development/github/scripts/git-credential-token.py"
```
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  export GITHUB_TOKEN=$(uv run python3 "${HERMES_HOME:-$HOME/.hermes}/skills/software-development/github/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
