...
例如，带有 `skill: arxiv` 的话题会在其会话重置时（手动 `/new` 或 `/reset`）预加载 arxiv 技能。
...
通过 `~/.hermes/config.yaml` 中的 `agent.clarify_timeout` 配置响应超时（默认 `3600` 秒）。如果你在超时内没有响应，Agent 会以哨兵消息解除阻塞并适应，而不是挂起。
