...
3. 主智能体已加载的技能（启动预加载或会话中通过 `skill_view` 加载）仅作为建议性来源记录。评审者根据其任务、所引用的制品、选定标准、评审者角色和明确依赖项确定所需上下文；不会仅因父智能体加载过某项技能就加载它。与所有子智能体一样，其系统提示也会嵌入工作区的项目上下文文件（AGENTS.md / CLAUDE.md / .cursorrules）作为具有约束力的约定。
