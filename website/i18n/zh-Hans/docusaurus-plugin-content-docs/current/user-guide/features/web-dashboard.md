...
这将启动一个本地 Web 服务器，并在浏览器中打开 `http://127.0.0.1:9119`。Dashboard 界面由本地服务器提供；agent 请求和工具仍会连接已配置的服务提供方。

:::danger 请将 Dashboard 视为高权限远程 Shell
Dashboard 并非只读状态页。它包含完整的 agent 和终端、工具执行、配置与
密钥管理，以及安装级操作。任何获得 Dashboard 访问权限的人，都能以
Hermes 进程相同的操作系统权限执行操作；如果 Hermes 以 `root` 运行，
这就意味着可以完全控制主机。请将其限制在回环地址或可信私有网络，
所有远程绑定都必须启用鉴权，并且不要以 `root` 运行对外暴露的 Dashboard。
:::
...
:::warning 安全提示
Web Dashboard 会读写包含 API 密钥和机密的 `.env` 文件，以 Hermes
进程的操作系统权限运行 agent 工具和终端命令，并执行安装级操作。
它默认绑定到 `127.0.0.1`——只能从本机访问且无需登录。绑定到包括
`0.0.0.0` 在内的任何非回环地址都会启用鉴权门控；在配置鉴权提供方
（用户名/密码或 OAuth）之前，服务器会拒绝启动。鉴权是强制要求，
但不会降低登录后获得的权限；配置公网 URL 时，代理后的回环后端也会启用鉴权。
不要以 `root` 运行对外暴露的 Dashboard。
:::
