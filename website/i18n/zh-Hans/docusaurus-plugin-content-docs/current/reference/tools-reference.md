...
**当前注册表快速统计：** 约 86 个工具 —— 10 个核心浏览器工具 + 2 个 CDP 门控浏览器工具、4 个文件工具、4 个 Home Assistant 工具、2 个终端工具、12 个桌面 GUI 工具、2 个 Web 工具、5 个 Feishu 工具、7 个 Spotify 工具、5 个 Yuanbao 工具、12 个 kanban 工具、3 个项目工具、2 个 Discord 工具、3 个视频工具，以及若干独立工具（`memory`、`clarify`、`delegate_task`、`execute_code`、`cronjob`、`session_search`、`skill_view`/`skill_manage`/`skills_list`、`text_to_speech`、`image_generate`、`vision_analyze`、`video_analyze`、`todo`、`computer_use`、`x_search`）。
...
:::note 出站平台投递
`send_message` 不再注册为 agent 可调用工具。脚本/手动发送请使用 `hermes send`，定时任务输出请使用 cron 的 `deliver=...`，平台响应则由 gateway delivery 层处理。
:::
