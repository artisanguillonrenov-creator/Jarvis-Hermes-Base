...
| `GIT_EDITOR`, `GIT_SEQUENCE_EDITOR`, `GIT_TERMINAL_PROMPT`, `GCM_INTERACTIVE` | **Forced to `true` / `true` / `0` / `Never` inside every terminal-tool shell** — foreground commands on every backend (local, Docker, SSH, Modal, ...) and pipe-mode background processes; `pty=true` background processes are interactive by design and keep the inherited editor. Agent shells have no terminal to type into, so a bare `git commit`, `git rebase -i`, or an HTTPS credential prompt would otherwise hang until the tool timeout; with these set they fail fast ("Aborting commit due to empty commit message"). Inherited values are deliberately overridden. To use an editor for one command, put the override on the command itself: `GIT_EDITOR='sed -i ...' git commit`. `GIT_PAGER`/`PAGER` default to `cat` and `DEBIAN_FRONTEND` to `noninteractive`, but those respect an inherited value. |
