...
| `HERMES_NODE_TARGET_MAJOR` | Pin the automatic Hermes-managed Node.js runtime (`~/.hermes/node`) heal to an exact major version, overriding the default derived from this checkout's `package.json` `engines.node` range. Only affects the silent automatic heal path — `hermes doctor --upgrade-node` always installs its own computed target major (the newest `engines.node`-named one, or an explicit `--upgrade-node=MAJOR`) and ignores this variable. See [`hermes doctor`](./cli-commands.md#hermes-doctor). |
