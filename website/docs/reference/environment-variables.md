...
| `HERMES_CRON_RUN_HISTORY_RETENTION` | Number of recent SessionDB run rows retained per cron job (default: `50`). Set to `0` to clear a job's run history after each fire. Also configurable via `cron.run_history_retention` in `config.yaml`. |
