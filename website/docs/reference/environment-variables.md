...
| `HERMES_GIT_EXE_PATH` | **Windows only.** Override `git.exe` discovery used by desktop Git integration (worktrees, review, branch ops). Follows the same pattern and resolution order as `HERMES_GIT_BASH_PATH` (PortableGit under LOCALAPPDATA first, then system Git locations, then PATH). Useful to pin a specific git.exe when multiple are present. |
