...
| `HERMES_UID` | **Docker only.** Override the UID used by the container's `hermes` user so bind-mounted files such as `~/.hermes` remain accessible to the host user. Usually set to `$(id -u)` alongside `HERMES_GID`. |
| `HERMES_GID` | **Docker only.** Override the GID used by the container's `hermes` user so bind-mounted files such as `~/.hermes` remain accessible to the host group. Usually set to `$(id -g)` alongside `HERMES_UID`. |
