...
| `per_call_authorization` | string | HTTP | Explicit per-call credential override. The only accepted value is `fizko_person_access_token`, and only for canonical Streamable HTTP `https://mcp.fizko.ai/mcp`; invalid opt-ins disable that server. A token-bearing web turn overrides `Authorization`; turns from other channels keep the profile credential. |
