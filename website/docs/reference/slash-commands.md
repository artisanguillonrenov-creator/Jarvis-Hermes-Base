...
| `/memory [pending\|approve\|reject\|approval]` | Show the effective memory approval policy (general `write_approval` plus the unattended background replace/remove restriction), review staged writes, and toggle the gate. See [Controlling memory writes](/user-guide/features/memory#controlling-memory-writes-write_approval). |
...
| `/memory [pending\|approve\|reject\|approval]` | Show the effective memory approval policy (general `write_approval` plus the unattended background replace/remove restriction), review staged writes in chat, and toggle the gate with `/memory approval on\|off`. See [Controlling memory writes](/user-guide/features/memory#controlling-memory-writes-write_approval). |
