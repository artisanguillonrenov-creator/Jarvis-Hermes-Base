...
| `--runtime auto\|container\|native` | Where this profile's gateway will be supervised. Defaults to the `profiles.runtime` config key, whose own default is `auto`: follow the gateway serving the active profile. A containerized serving gateway means the new profile is treated as containerized too, and the printed start command hands off to `docker exec`. `auto` resolves to `native` wherever no gateway declares a container runtime. |
