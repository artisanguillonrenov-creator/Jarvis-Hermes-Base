...
| `kanban_attach` | Attach a file to a task by passing its bytes inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB. content_base64 must be mechanically encoded from the source file — path-like placeholder payloads are rejected by an integrity guard; the result echoes the stored sha256. | `HERMES_KANBAN_TASK` or `kanban` toolset |
| `kanban_attach_file` | Attach a local file by path — Hermes reads the bytes from disk (worktree/absolute/unicode paths all fine) and stores them byte-verified, recording the sha256 in the attachment row. Preferred for files already on disk. | `HERMES_KANBAN_TASK` or `kanban` toolset |
