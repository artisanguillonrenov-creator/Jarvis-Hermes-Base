...
| `repair-chains` | Detect orphaned compression segments (old builds wrote continuations as independent roots) and, with `--apply`, relink them under the oldest root of each group. Interactive checklist + automatic snapshot; weak-signal same-title groups are listed unchecked. See [Sessions → Repair Orphaned Compression Chains](../user-guide/sessions.md#repair-orphaned-compression-chains). |
| `retitle-missing` | Batch-regenerate missing or truncated session titles (roots via the title-generation model, empty chain segments by inheriting the ancestor title). Interactive checklist + automatic snapshot; user-titled rows are never touched. See [Sessions → Retitle Missing Session Titles](../user-guide/sessions.md#retitle-missing-session-titles). |
| `merge-chains` | Flatten fork compression chains back into one in-place session, matching modern in-place compression. Moves segment messages onto the head, merges token/cost counters and usage rows; automatic snapshot. See [Sessions → Merge Fork Compression Chains](../user-guide/sessions.md#merge-fork-compression-chains). |
| `restore-db` | Restore `state.db` from a `.pre-*` snapshot (auto-taken by the maintenance commands). Stops processes holding the database, integrity-checks the snapshot before and after the swap; the chosen snapshot is never skippable. Stopped processes are not auto-restarted — their restart commands are printed. See [Sessions → Restore the Session Database](../user-guide/sessions.md#restore-the-session-database). |
