...
| `install --upgrade` | Re-run the installer even if Hermes already resolves cua-driver through its canonical resolver. The upstream script always pulls the latest release, so this performs an in-place upgrade. |
...
of a confirmed update if Hermes resolves cua-driver through the same canonical
resolver used at runtime, so most users will not need
