...
| `open_codereview` | `open_codereview` | Review a diff through [Open Code Review](https://open-codereview.ai)'s local command line tool, and mirror Hermes's active provider/model into that tool so switching models never means re-configuring it by hand. Off by default — opt in via `hermes tools` → 🔎 Open Code Review. Schema is only registered when `open_codereview.command` resolves (check_fn-gated). |
