...
### Register an optional content-search backend

Plugins that accelerate file content search without replacing the `search_files` tool can register an advisory backend:

```python
from hermes_cli.search_backends import SearchBackendDecline, SearchBackendResult


def search(request):
    if not request.is_local or request.context:
        return SearchBackendDecline("unsupported_request")
    # Return validated matches/files/counts, or decline to native rg/grep.
    return SearchBackendResult(
        backend="my-index", route_reason="eligible", matches=[], total_count=0)


def register(ctx):
    ctx.register_search_backend("my-index", search)
```

The seam is intentionally narrow and content-search-only. Hermes performs path existence checks before invocation and retains result containment validation, blocked-path filtering, redaction, pagination output shaping, and native rg/grep fallback. A callback exception, malformed result, out-of-root path, `SearchBackendDecline`, or `None` cannot replace those contracts; Hermes falls back to native search. Registrations are scoped to the active profile and are disposed on plugin unload. No `tools.override` capability is required.
