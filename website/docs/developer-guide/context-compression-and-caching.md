...
compression:
  ...
  tail_assistant_max_chars: 0  # lean, opt-in: cut tail assistant content over N chars (default: 0 = off)
...
| `tail_assistant_max_chars` | `0` | ≥0 | `lean` only, opt-in. Cut an assistant message in the tail whose content exceeds N characters, replacing the overflow with a `session_search` recovery pointer. A degenerate generation (a model that collapses into repeating one phrase) writes a single message far larger than the whole tail budget; neither the message-count floor nor the newest-assistant anchor can evict it, so compaction reclaims nothing and every later turn re-sends it. `tool_calls` are never touched (tool-group pairing survives) and the full text stays in session history. Off by default because the cut rewrites text the model really emitted; `16000` (~4K tokens) is what the deployment that hit this ran. Lowering a live cap re-cuts messages an earlier pass already truncated |
