...
- **`on_turn_complete()`** is post-turn observation only; treat `messages` as read-only. **Coverage is best-effort:** it fires from the standard turn-finalization seam and from content-policy and provider-terminal exits that persist and return directly from the retry loop. Other abnormal early-return paths may still skip it, so it is not a guaranteed callback for every possible exit.
