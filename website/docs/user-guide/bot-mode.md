...
When you register several backends in **Settings → Connections** — the local runtime, remote gateways, SSH hosts, Hermes Cloud instances — the roster shows the Bots from **every** connected source: remote gateways are inventoried without spawning anything on the remote box, and machines that are momentarily unreachable keep their last-known rows instead of vanishing. The local runtime ("This device") and SSH sources are **connect-on-demand**: when the desktop's primary connection is a remote gateway, the local source's bots only appear once a local backend is live (opening any local bot starts one), and they remain for the rest of the session; after an app restart they are hidden again until a local bot is opened. When the same profile name exists on several sources, handles disambiguate as `@name-device` (for example `@research-homelab`). A Bot's chats, sessions, memory, and routines live on the machine that owns the profile.
