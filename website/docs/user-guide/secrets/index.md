...
## File blobs are not vault items or source-apply material

Passport scans, bank statements, and tax PDFs (for example 1099s) are file blobs. They are **not** `hermes vault add` / Passwords & Logins items, and they are **not** Bitwarden, 1Password, or command source-apply material. Do not paste them into chat. Do not `read_file` them for the model. Destination-bound file tokens (Skyflow / VGS) stay **out of tree** — ship them as plugins and share them in the Nous Research Discord (`#plugins-skills-and-skins`), same placement as SecretSource plugins. Bank login and transaction context belong on an aggregator path (Plaid), not stuffing a statement PDF into the vault.
