...
compression:
  ...
  context_total_ceiling_seconds: 600                # Cap on pre-stream in-agent compress_context work; active streams use provider guards
...
`context_total_ceiling_seconds` (default `600`) bounds in-agent work until the summary provider produces substantive output. It is clamped to at least `context_timeout_seconds`. Once a summary stream is active, the provider stream's inactivity and hard-ceiling guards take over, so a healthy slow local model is not cancelled merely because the host's startup ceiling elapsed. Once the worker has entered the compression commit fence and SessionDB mutation is in flight, the commit is never abandoned mid-flight — that would risk transcript divergence — but the wait is no longer silent: if the commit runs past the ceiling, Hermes logs the overrun (WARNING, escalating to ERROR on repeat), sends a one-shot warning through the user-visible warning channel, and keeps waiting in bounded increments until the commit completes. If pre-stream work expires, every auxiliary wire closes at that same instant, so an abandoned request is not left running.
