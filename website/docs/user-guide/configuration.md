...
delegation:
  ...
  #     provider:
  #       sort: throughput
  max_concurrent_children: 10               # Parallel children per batch (floor 1, no ceiling). Also via DELEGATION_MAX_CONCURRENT_CHILDREN env var.
  ...
  max_spawn_depth: 1                        # Delegation tree depth cap (floor 1, no ceiling). 1 = flat (default): parent spawns leaves that cannot delegate. 2 = orchestrator children can spawn leaf grandchildren. 3+ = deeper levels.
