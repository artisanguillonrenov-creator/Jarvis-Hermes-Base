...
agent:
  ...
  api_max_retries: 3           # Base attempts per API retry cycle, including the first call (minimum: 1)
```
...
`agent.api_max_retries` sets the base number of attempts in each Hermes-level API retry cycle, **including the first call**. The default is `3`: one initial attempt and up to two retries in that cycle. Set it to `1` to shorten the cycle; `0` and negative values are clamped to `1`.

This is not a limit on total network requests. SDK retries and streaming retries have separate budgets. Provider-specific error handling can adjust the budget or activate a [fallback provider](/user-guide/features/fallback-providers) early. For eligible transport errors, Hermes can rebuild the primary client once and start another retry cycle before falling back, so `1` does not guarantee immediate failover after the first error.
