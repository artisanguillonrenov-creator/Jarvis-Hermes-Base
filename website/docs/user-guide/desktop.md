...
- **Every provider and model in the menus** — the GUI surfaces the full provider list (minus anything in [`model_catalog.excluded_providers`](/reference/model-catalog#hiding-providers-from-the-picker), which hides the same providers here as in `hermes model`) and every model that `hermes model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
