...
- **Code block height** — **Settings → Appearance → Code Block Height** sets how tall a chat code block may grow before it folds behind an expand control: **Compact** (about seven lines, the default), **Tall** (about twenty), or **Off** (never fold, so long blocks stay fully selectable). Blocks long enough to become artifacts (48+ lines or 3000+ characters with a language tag) open as artifact cards and are not affected. This is a per-computer setting.
