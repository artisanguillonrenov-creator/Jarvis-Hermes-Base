...
- **Customizable items** — right-click the status bar (**Show in status bar**) to choose what appears: the context meter, a provider account-limits/quota chip with a details popover (shown when the active provider supports it), cache hit rate, tokens per second, workspace, model, approvals, turn/session timers, terminal, Command Center, backend version, and more — or hide the bar entirely (**Cmd/Ctrl+Shift+S** toggles it).
