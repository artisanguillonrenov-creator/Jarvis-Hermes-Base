...
- **Pull-request badges** — when a session branch has a GitHub pull request, Desktop shows its state and number beside the branch; click the badge to open the PR. This is read-only discovery through the GitHub CLI and requires `gh` to be installed and authenticated on the machine that owns the checkout (the remote backend when Desktop is connected remotely). Sessions that created a PR from a worktree can also recover the badge from the bare PR URL printed by `gh pr create`. If `gh` is unavailable, unauthenticated, the repository has no GitHub remote, or no matching PR exists, Desktop simply omits the badge. Trunk branches (`main`, `master`, `develop`, `dev`, `trunk`) are not matched by branch name, which avoids attaching an unrelated fork PR to a trunk session.
