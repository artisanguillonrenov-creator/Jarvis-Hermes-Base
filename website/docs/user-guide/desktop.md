...
- **Restack and per-commit review** — when the branch has commits of its own, the review pane grows a **Restack** button and a **Commit to review** picker. **Restack** hands the agent the branch's exact commit range (merge base → HEAD, plus any uncommitted changes) and asks it to rewrite that history into a clean, logically ordered set of commits: unrelated changes split apart, fixups and WIP squashed into the commit they belong to, every commit given a proper message. The final file contents stay byte-identical (the agent snapshots the tree first and verifies with `git diff` at the end); nothing is pushed. When the agent's turn settles the picker re-reads the stack, and choosing a commit shows just that commit's patch, so you can review a large diff one logical piece at a time before shipping.
