...
- **Restore a checkpoint** — rerun from an earlier user prompt, discarding the later conversation. This also works for prompts preserved by in-place compaction in the same stored session: Hermes restores the retained transcript prefix from the session database before rerunning the prompt. Ordinary resume still uses the compressed working context. Restore rewinds the conversation, not files or external actions performed by its tools.
