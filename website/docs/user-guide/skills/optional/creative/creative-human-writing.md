---
title: "Human Writing"
sidebar_label: "Human Writing"
description: "Natural-sounding Chinese prose (活人感写作): write or revise Chinese long-form content — Zhihu answers, forum posts, WeChat articles, blogs, stories, scripts — so..."
---
...
Natural-sounding Chinese prose (活人感写作): write or revise Chinese long-form content — Zhihu answers, forum posts, WeChat articles, blogs, stories, scripts — so it reads like a real person, not a model. Enforces material-first drafting and hard bans on AI-tell rhetoric (翻案句, 破折号滥用, 黑话), with an automated checker script.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hermes skills install official/creative/human-writing` |
| Path | `optional-skills/creative/human-writing` |
| Version | `1.1.0` |
| Author | KKKKhazix (https://github.com/KKKKhazix/human-writing), ported by Hermes Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `writing`, `chinese`, `zh-CN`, `prose`, `anti-ai-slop`, `editing`, `copywriting`, `humanize` |
...
> Ported from [KKKKhazix/human-writing](https://github.com/KKKKhazix/human-writing) v1.1.0 (MIT).
> Scope: CHINESE prose. For English anti-AI-slop editing, use the bundled `humanizer` skill instead.
>
> Hermes adaptation notes:
> - Where the skill says to research/检索, use `web_search` / `web_extract`.
> - Run the hard-rule checker on a saved draft via terminal:
>   `python3 scripts/check_prose.py <draft.md>` (from this skill's directory; stdin via `-` also works).
>   The script only enforces the explicit bans below (翻案句/黑话/破折号/冒号/名词化/洞察路标 etc.); style judgment stays with you.
> - Load reference files per the "按任务读取" section — only the ones the current task actually hits.
> - If `read_file` reports "binary file" on the dense-CJK reference files, read them via terminal
>   (`cat references/forum-prose.md`) — they are plain UTF-8; the binary heuristic misfires on CJK-heavy text.
> - Checker output has three tiers. Only items under `需要修改` block delivery; the count line may show
>   nonzero soft-marker counts (e.g. `洞察路标 1`) while still concluding clean — those are below the
>   length-scaled warning threshold and need no action.
