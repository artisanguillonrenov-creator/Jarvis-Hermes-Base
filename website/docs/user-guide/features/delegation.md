...
3. Parent-loaded skills (launch-preloaded or loaded through `skill_view`) are included only as advisory provenance. The reviewer derives required context from its assignment, the referenced artifact, selected criteria, reviewer role, and explicit dependencies; it does not load a skill solely because the parent did. Like every subagent, its system prompt also embeds the workspace's project context files (AGENTS.md / CLAUDE.md / .cursorrules) as binding conventions.
