...
Over REST, `POST /v1/mcp/reload` on the [API server](api-server.md#mcp-reload) does the same for one agent (`/p/<profile>/v1/mcp/reload` under multiplex), which is how an orchestrator makes a rotated `${VAR}` credential take effect without a restart.
