...
| `agent:end` | Agent finishes processing, or is abandoned before it can | same keys as `agent:start`, plus `response` (truncated to 500 chars), `model` and `provider`. Emitted exactly once per attempted `agent:start`: a turn abandoned before it produced a response — it raised, was cancelled, was superseded, or its heartbeat owner moved on — still emits it, with empty `response`/`model`/`provider` and `failed` (`true`). `failed` is present only on those abandoned emits; a turn whose result reports a failure emits the normal payload |
