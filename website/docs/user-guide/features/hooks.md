...
| `poll_vote` | telegram | `poll_id: str`, `option_ids: list[int]` (0-based, may be empty on retraction), `option_texts: list[str]` (resolved from the send-time options list; empty when unresolvable), `question: str`, `user: {id, name}`, `chat_id: str \| None`, `thread_id: str \| None`, `message_id: str \| None` (origin from the adapter's bounded sendPoll cache — an unknown or evicted `poll_id` never fires, the vote is logged at WARNING and dropped), `update_id: int \| None`. Votes on polls the bot sent are *also* persisted into the owning session as a `[Poll vote] ...` user-role turn regardless of whether any plugin subscribes to this hook. |
