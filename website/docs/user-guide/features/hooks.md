...
| `gateway_platform_event` | Observer/action | After the gateway's profile-scoped authorization succeeds, when a supported platform-native event is normalized at the gateway boundary. Observer returns are ignored; a callback-query plugin may return one `{"action": "handled", ...}` envelope. | `platform`, `event_type`, `payload` (event-type-specific dict — see the per-event contracts below) | Normalized plain-dict envelope only; raw SDK objects, adapter handles, and bot clients are never exposed. |
...
| `callback_query` | telegram | `data: str`, `chat_id: str`, `message_id: str \| None`, `message_text: str`, `user_id: str`, `user_name: str \| None`. Plugins may return an action envelope: `{"action": "handled", "answer": str \| None, "edit": {"text": str, "buttons": [[{"label": str, "data": str}]] \| [] \| None}}`. Callback data should use a plugin-specific namespace; built-in prefixes such as `mp:`, `mpg:`, `mpv:`, `mm:`, `mc:`, `mb`, `mx`, `mg:`, `cp:`, `gt:`, and `ea:` are reserved by the Telegram adapter. |
