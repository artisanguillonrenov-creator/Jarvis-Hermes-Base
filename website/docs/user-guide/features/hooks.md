...
|--------------|-----------|----------------|
| `reaction` | telegram | `emojis: list[str]` (the message's full current reaction set), `added_emojis: list[str]` (only the emoji newly added by this update — consumers that act once per reaction should read this, not `emojis`), `custom_emoji_ids: list[str]`, `chat_id: str`, `message_id: str`, `thread_id: str \| None` (Telegram reaction updates carry no topic id, so currently always `None`). |
