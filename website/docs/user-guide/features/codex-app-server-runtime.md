...
- **`kanban_create` / `kanban_link`** — orchestrator-only operations. Available for orchestrator agents running on the codex runtime that need to dispatch new tasks. `kanban_unblock` is available to orchestrators for any task and to workers for other tasks (never their own; evidence required).
