...
Set `composition_only: true` for a bounded, no-tools composition run. Hermes disables memory and context-file injection, exposes no callable tools, and allows one logical agent iteration. The mode fails closed when the configured runtime is `codex_app_server`; normal runs are unchanged. Clients can feature-detect support through `features.composition_only_runs` in `GET /v1/capabilities`.
