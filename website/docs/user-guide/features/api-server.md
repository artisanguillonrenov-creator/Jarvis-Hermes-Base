...
Under [`gateway.multiplex_profiles`](../multi-profile-gateways.md) address the agent with the `/p/<profile>/` prefix: only that profile's servers are torn down and rediscovered, the other profiles' connections are untouched. `ok: false` (HTTP 500) carries the reload's own error text in `summary`. The endpoint is gated by the profile's `API_SERVER_KEY` and does not go through the chat-side `approvals.mcp_reload_confirm` prompt — the key is the authorization. Advertised as `endpoints.mcp_reload` in `/v1/capabilities`.
