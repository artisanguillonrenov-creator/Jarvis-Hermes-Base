...
hermes kanban unblock <id>... [--takeover]
...
hermes kanban reopen-review  <id>... [--reason "..."] [--takeover]    # changes requested: 'review' -> ready/todo
...
| `respawn_guarded` | `{reason}` (`{reason, pid, host, run_id}` for `prev_worker_alive`/`prev_worker_cross_host_unknown`) | Dispatcher refused to re-spawn this ready/review task this tick. Reasons: `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (a GitHub PR URL appears in a recent comment — a prior worker already opened a PR), `prev_worker_alive` (the closing run's worker pid is still alive on this host — defence in depth against a duplicate writer), `prev_worker_cross_host_unknown` (the run closed on a different host with an outcome where a live remote worker is plausible, and this host cannot verify a remote pid's liveness). The task stays in `ready`/`review`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures — except `prev_worker_alive`/`prev_worker_cross_host_unknown`, which escalate separately (`respawn_guard_escalated`) after 5 consecutive ticks. |
| `prev_worker_ack` | `{run_id, reason}` | `hermes kanban unblock --takeover` / `reopen-review --takeover` durably retired a `prev_worker_alive`/`prev_worker_cross_host_unknown` guard's evidence for the named closed run, after a human confirmed out-of-band that the previous worker is genuinely gone. Stamped on the run row's own metadata (`prev_worker_ack: true`) atomically with the `blocked`/`review` → `ready` flip — a failed flip leaves the guard untouched. Refused outright when the hold is same-host `prev_worker_alive` AND the pid is still directly verifiable as alive from this host (the cross-host reason keeps the unconditional escape hatch, since this host cannot check a remote pid). Nothing automatic emits this event; only an explicit `--takeover` does, so the fail-closed default is unweakened for every card that was never acknowledged. |
