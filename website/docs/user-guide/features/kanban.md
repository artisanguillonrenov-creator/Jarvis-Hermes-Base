...
| Key | Default | Purpose |
|---|---|---|
| `auto_decompose` | `false` | Set to `true` to let the dispatcher auto-run the built-in decomposer for Triage tasks every tick. It does not gate profile-driven `kanban_create` calls or creator wake turns. |
