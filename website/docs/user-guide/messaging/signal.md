...
**Reply quotes.** When Hermes replies to a specific message, its text and attachment sends use a native reply that quotes the original — the same UI affordance Signal users see when they use "Reply" themselves. This is automatic for replies generated in response to an inbound message.
