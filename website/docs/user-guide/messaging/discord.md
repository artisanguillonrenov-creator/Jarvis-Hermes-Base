...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Hermes. `"all"` — accept all bot messages. Also gates inbound reactions: a bot's reaction on a Hermes message is routed only under `"all"`, since a reaction carries no text and can never satisfy `"mentions"`. |
| `DISCORD_REACTIONS` | No | `true` | When `true`, the bot adds emoji reactions to messages during processing (👀 when starting, ✅ on success, ❌ on error), and routes reactions users add to its own messages into the agent as a `reaction:added:<emoji>` / `reaction:removed:<emoji>` turn. Set to `false` to disable reactions entirely, in both directions. |
