...
Hermes loads `AGENTS.md` as a directory chain at session start: from the git root down to the current working directory, with deeper files taking precedence. Subdirectory `AGENTS.md` files are discovered lazily during tool calls (via `subdirectory_hints.py`) and injected into tool results — they are not loaded upfront into the system prompt.
