...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    async def _handle_message(
        ...
        if not is_thread and not isinstance(message.channel, discord.DMChannel):
            ...
            # Free-response channels skip auto-threading by default; opt back in per-profile via
            # config.yaml ``discord.auto_thread_free_response`` / DISCORD_AUTO_THREAD_FREE_RESPONSE.
            auto_thread_free = self._extra_or_env_flag(
                "auto_thread_free_response", "DISCORD_AUTO_THREAD_FREE_RESPONSE", "false", truthy=True
            )
