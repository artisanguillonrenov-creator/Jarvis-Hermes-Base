...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def _approval_mention_content(self) -> Optional[str]:
        ...
        (``DISCORD_APPROVAL_MENTIONS``). Secondary profiles resolve scoped env first,
        then ``PlatformConfig.extra``, never another profile's process-global value.
        The unscoped multiplex default adapter may use env when config is absent;
        single-profile keeps env-over-config precedence. Only numeric allowlist entries; default off."""
        extra = getattr(getattr(self, "config", None), "extra", None)
        ...
        if _multiplex_active():
            try:
                ...
                if scope is not None:
                    ...
                    # The multiplex default adapter has no secondary-profile
                    # scope. Reuse upstream's gate reader for its env-only opt-in;
                    # an explicit profile config still wins over process env.
                    raw_enabled = _scoped_gate_env("DISCORD_APPROVAL_MENTIONS")
            except Exception:
                pass
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    # approval_mentions is a per-profile gate too: seed extra so multiplexed adapters read their own
    # profile's value rather than another profile's process-global env.
    _gate("approval_mentions", "DISCORD_APPROVAL_MENTIONS", from_platform_extra=True, lower=True)
