...
class TestAdapterCapabilityFlag:
    ...
    def test_api_server_bind_chokepoint_hardwires_no_delivery(self):
        ...
        from gateway.session_context import clear_session_vars, delegation_delivery_mode, get_session_env
        ...
        try:
            assert async_delivery_supported() is False
            assert get_session_env("HERMES_SESSION_PLATFORM") == "api_server"
            assert delegation_delivery_mode() == "background"
        finally:
            clear_session_vars(tokens)
        assert delegation_delivery_mode() == "background"

        tokens = APIServerAdapter._bind_api_server_session(
            chat_id="c2", session_key="sk2", session_id="sid2", delegation_delivery="join")
        try:
            assert delegation_delivery_mode() == "join"
        finally:
            clear_session_vars(tokens)
        assert delegation_delivery_mode() == "background"
