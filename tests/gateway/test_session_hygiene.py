...
@pytest.mark.asyncio
async def test_session_hygiene_turn_hold_notice_is_silent_when_progress_notices_off(
    monkeypatch, tmp_path
):
    """A compression that still streams progress must not hold the turn hostage.
    ...
    # The deferral notice is routine progress and honours compression.progress_notices;
    # this test asserts the notice itself, so enable the gate.
    monkeypatch.setattr("gateway.run._gateway_compression_progress_notices_enabled", lambda: False)
    ...
    assert not any(
        "deferred" in c.lower() or "still streaming" in c.lower()
        for c in sent_contents
    ), f"turn-hold notice must stay silent when progress_notices is off, got: {sent_contents}"
