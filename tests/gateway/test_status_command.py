...
@pytest.mark.asyncio
async def test_subsequent_empty_session_skips_home_channel_prompt(monkeypatch):
    """A new empty chat is not first-ever install — do not nag for /sethome."""
    import gateway.run as gateway_run
    ...
    assert result == "ok"
    for call in runner.adapters[Platform.SLACK].send.await_args_list:
        content = call.args[1] if len(call.args) > 1 else ""
        assert "No home channel is set" not in content
