...
class TestCleanPlugin:
    ...
    def test_benchmarking_fixture_critical_caps_but_runtime_secret_still_blocks(self, tmp_path):
        ...
        critical there caps like a test tree: visible at `high`, verdict
        `caution` — blocked by default, `--force` overridable. The same shape
        ...
        assert result.verdict == "caution", [(f.pattern_id, f.severity, f.file) for f in result.findings]
        assert any(
            f.pattern_id == "embedded_private_key" and f.severity == "high" and f.file == "benchmarking/stress.py"
