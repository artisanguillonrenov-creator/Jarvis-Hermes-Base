...
def test_popen_bash_uses_kill_on_exit_wrapper(monkeypatch):
    """tools.environments.base_output._popen_bash must route its Popen call through
    ...
    from tools.environments import base_output as env_base
    from hermes_cli import _subprocess_compat
...
def test_spawn_local_background_shell_uses_kill_on_exit_wrapper(tmp_path):
    ...
    Call-contract check in the same style as the base_output.py test: the wrapper
