...
def test_complete_task_reaps_clean_worktree(kanban_home: Path, repo: Path) -> None:
    with kbc.connect_closing() as conn:
        ...
        assert kb.complete_task(conn, tid, summary="done", expected_run_id=kb._current_run_id(conn, tid))
    assert not wt.exists()
...
def test_parent_worktree_deferred_until_children_done(
    ...
    with kbc.connect_closing() as conn:
        ...
        assert kb.complete_task(conn, parent, summary="parent done", expected_run_id=kb._current_run_id(conn, parent))
        ...
        assert kb.complete_task(conn, child, summary="child done", expected_run_id=kb._current_run_id(conn, child))
