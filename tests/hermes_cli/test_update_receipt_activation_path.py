...
swallowed twice (``finalize_update_receipt``'s own ``except`` → DEBUG, then
``update_cmd_common._best_effort`` → DEBUG), so a failure is invisible and unrecoverable, and the
...
class TestWriteFailureIsVisible:
    def test_unresolvable_receipt_home_warns_instead_of_debug(
        ...
        """The field shape: resolving the receipt home re-imports hermes_cli.config, which
        imports a symbol the pre-pull `utils` lacks. That used to be swallowed at DEBUG."""
        def boom():
