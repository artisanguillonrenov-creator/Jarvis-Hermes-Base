...
def test_recompute_ready_no_event_when_update_affects_zero_rows(kanban_home):
    """A promotion whose UPDATE lands on zero rows must not be reported as a
    success (#77140): no ``promoted`` event, no count.

    Zero-row UPDATEs are real — deployments guard rows with ``BEFORE UPDATE``
    triggers that ``RAISE(IGNORE)``, and a concurrent writer can flip the
    status between the candidate SELECT and the CAS UPDATE. Before the
    rowcount gate, one fleet logged 623k fabricated ``promoted`` events in
    12 hours while the tasks never left ``todo``.
    """
    with kbc.connect() as conn:
        t = kb.create_task(conn, title="suppressed", assignee="a")
        # Park the task in the dispatcher's intake status first (create_task
        # lands parentless tasks directly in ready).
        conn.execute("UPDATE tasks SET status='todo' WHERE id=?", (t,))
        # SQLite DDL takes no bind parameters; the id is generated by
        # create_task, so splicing it into the WHEN clause is safe here.
        conn.execute(
            f"CREATE TRIGGER suppress_update BEFORE UPDATE ON tasks "
            f"WHEN OLD.id = '{t}' BEGIN SELECT RAISE(IGNORE); END"
        )
        conn.commit()

        # The UPDATE inside recompute_ready is silently ignored (rowcount 0).
        assert kb.recompute_ready(conn) == 0
        ...
        # Control: without the guard the same task promotes normally.
        conn.execute("DROP TRIGGER suppress_update")
        conn.commit()
