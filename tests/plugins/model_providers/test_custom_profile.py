...
These tests pin the wire-shape contract:
    ...
    - disabled elsewhere  → omit reasoning controls (strict APIs 400/422)
...
class TestCustomReasoningWireShape:
    ...
    def test_disabled_omits_reasoning_controls_on_mistral(self, custom_profile):
        """Strict OpenAI-compat hosts reject ``reasoning_effort='none'``."""
        eb, tl = custom_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": True, "effort": "none"},
    ...
    def test_disabled_omits_reasoning_controls_without_base_url(self, custom_profile):
        """Unknown custom endpoint — do not guess Ollama-specific controls."""
        eb, tl = custom_profile.build_api_kwargs_extras(
    ...
    def test_disabled_omits_reasoning_controls_on_non_ollama_relays(self, custom_profile, base_url):
        eb, tl = custom_profile.build_api_kwargs_extras(
    ...
    def test_disabled_lan_vllm_preserves_template_kwargs_without_none_effort(self, custom_profile):
        """Continuation retries must not contradict user chat-template kwargs."""
        from agent.transports.chat_completions import ChatCompletionsTransport

        api_kwargs = ChatCompletionsTransport().build_kwargs(
            "qwen3.8-27b",
            [{"role": "user", "content": "hello"}],
            provider_profile=custom_profile,
            reasoning_config={"enabled": False, "effort": "none"},
            base_url="http://192.168.0.41:30000/v1",
            request_overrides={"extra_body": {"chat_template_kwargs": {"enable_thinking": True}}},
        )

        assert "reasoning_effort" not in api_kwargs
        assert api_kwargs["extra_body"] == {"chat_template_kwargs": {"enable_thinking": True}}
