"""#110831: a fallback destination that only resolved with the keyless free-tier placeholder
(or no key at all) on a paid, remote provider with an empty credential pool must be skipped
instead of committed — the committed switch traded a "model unavailable" 400 for an endless
401 loop on a pool with nothing to rotate. Keyless overlays, local endpoints, pool
credentials, and entry-declared or client-held keys still activate.
"""
...
# The keyless free-tier placeholder is what a paid-provider entry holding only a *-free slug
# resolves with (#110831).
_PLACEHOLDER = "opencode-zen-free" + "-keyless"
...
def test_unauthenticated_destination_is_skipped_instead_of_committed():
    agent = _make_agent(fallback_model={"provider": "opencode", "model": "deepseek-v4-flash-free"})
    agent.provider, agent.model = "opencode-free", "deepseek-v4-flash-free"
    with (
        patch("agent.auxiliary_client.resolve_provider_client",
              return_value=(_fb_client("https://opencode.ai/zen/v1", client_key=_PLACEHOLDER), None)),
        patch("agent.credential_pool.load_pool", return_value=_pool(False)),
    ):
        assert agent._try_activate_fallback() is False
    assert (agent.provider, agent.model) == ("opencode-free", "deepseek-v4-flash-free")
...
def test_keyless_overlay_destination_activates_without_credentials():
    agent = _make_agent(fallback_model={"provider": "opencode-free", "model": "mimo-v2.5-free"})
    with (
        patch("agent.auxiliary_client.resolve_provider_client",
              return_value=(_fb_client("https://opencode.ai/zen/v1", client_key=_PLACEHOLDER), None)),
        patch("agent.credential_pool.load_pool", return_value=_pool(False)),
        ...
        assert agent._try_activate_fallback() is True
    assert (agent.provider, agent.model) == ("opencode-free", "mimo-v2.5-free")
