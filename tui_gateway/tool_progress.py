...
projection. Reaches server.py state through ``srv`` (method_ctx.py)."""
...
def _cap_tui_verbose_text(text: str) -> str:
    if len(text) <= srv._TUI_VERBOSE_TEXT_MAX_CHARS and text.count("\n") < srv._TUI_VERBOSE_TEXT_MAX_LINES:
        return text
    # Start of the last MAX_LINES lines, then pull forward to the char budget (never mid-line).
    line_start = len(text) - len("\n".join(text.split("\n")[-srv._TUI_VERBOSE_TEXT_MAX_LINES:]))
    start = max(line_start, len(text) - srv._TUI_VERBOSE_TEXT_MAX_CHARS)
    if start > line_start:
...
def _redact_tui_verbose_text(text: str) -> str:
    try:
        ...
        return ""
    return srv._cap_tui_verbose_text(redacted)
...
def _verbose_text(render, fallback) -> str:
    ...
    try:
        ...
        raw = fallback()
    return srv._redact_tui_verbose_text(raw)


def _tool_args_text(args: dict) -> str:
    return srv._verbose_text(lambda: json.dumps(args or {}, indent=2, ensure_ascii=False, default=str), lambda: str(args or {}))
...
def _tool_result_text(result: object) -> str:
    ...
    return srv._verbose_text(render, lambda: str(result))
...
def _cache_todo_state(session: dict, state: dict | None) -> None:
    ...
    cached = srv._normalize_todo_state(session.get("todo_state")) if state is not None else None
...
def _session_todo_state(session: dict) -> dict | None:
    ...
    cached = srv._normalize_todo_state(session.get("todo_state"))
    live = None
    snapshot = getattr(getattr(session.get("agent"), "_todo_store", None), "snapshot", None)
    if callable(snapshot):
        try:
            live = srv._normalize_todo_state(snapshot())
        except Exception:
...
def _attach_todo_state(payload: dict, session: dict) -> dict:
    ...
    state = srv._session_todo_state(session)
...
def _todo_state_from_history(history) -> dict | None:
    ...
    try:
        ...
        todo_call_ids = {
            ...
            if (call.get("function") or {}).get("name") in srv._TODO_TOOL_NAMES and call.get("id")
        }
        ...
        for msg in reversed(history):
            ...
            if not isinstance(content, str) or len(content) > MAX_TODO_RESULT_CHARS or '"todos"' not in content:
                continue
            try:
                return srv._normalize_todo_state(json.loads(content))
            except Exception:
                continue
        return None
    except Exception:
...
def _connector_lifecycle_is_stale(sid: str, name: str, args: dict) -> bool:
    if not srv._connector_tool_lifecycle(name, args):
        return False
    owner = srv._current_runtime_session_record.get()
    return owner is not None and (srv._sessions.get(sid) is not owner or owner.get("_finalized", False))


def _emit_tool_lifecycle(event, sid, name, args, payload):
    payload_model = payload
    if not srv._connector_tool_lifecycle(name, args):
        return srv._emit(event, sid, payload_model)
    ...
    with srv._sessions_lock:
        if srv._connector_lifecycle_is_stale(sid, name, args):
            return
        transport = (srv._sessions.get(sid) or {}).get("transport")
        if transport is None:
            transport = srv.current_transport() or srv._stdio_transport
    frame = srv._event_frame(event, sid, payload)
    _stamp_event(frame)
    from tui_gateway.hosted_room_member_activity import project_room_member_activity
    project_room_member_activity(frame, srv._sessions)
...
def _on_tool_start(sid: str, tool_call_id: str, name: str, args: dict):
    ...
    if (srv._tool_progress_enabled(sid) or srv._tool_lifecycle_required_for_ui(name)
            or srv._connector_tool_lifecycle(name, args)):
        payload: dict[str, object] = {"tool_id": tool_call_id, "name": name, "context": srv._tool_ctx(name, args)}
        ...
        if args:
            payload["args"] = args
        if srv._session_verbose(sid) and (args_text := srv._tool_args_text(args)):
            payload["args_text"] = args_text
        srv._emit_tool_lifecycle("tool.start", sid, name, args, ToolStartPayload.model_validate(payload))
...
def _on_tool_complete(sid: str, tool_call_id: str, name: str, args: dict, result: str):
    ...
    summary = srv._tool_summary(name, result, duration_s)
    if summary:
        payload["summary"] = summary
    if srv._session_verbose(sid) and (result_text := srv._tool_result_text(result)):
        payload["result_text"] = result_text
    todo_state = srv._normalize_todo_state(payload.get("result")) if name in srv._TODO_TOOL_NAMES else None
    if todo_state is not None:
        payload.update(todo_state)
        if session is not None:
            srv._cache_todo_state(session, todo_state)
    with contextlib.suppress(Exception):
        from agent.display import render_edit_diff_with_delta
        rendered: list[str] = []
        if render_edit_diff_with_delta(name, result, function_args=args, snapshot=snapshot, print_fn=rendered.append):
            payload["inline_diff"] = "\n".join(rendered)
    if (srv._tool_progress_enabled(sid) or payload.get("inline_diff") or srv._tool_lifecycle_required_for_ui(name)
            or name in srv._TODO_TOOL_NAMES or srv._connector_tool_lifecycle(name, args)):
        srv._emit_tool_lifecycle("tool.complete", sid, name, args, ToolCompletePayload.model_validate(payload))
    ...
    if todo_state is not None:
        srv._emit("todo.updated", sid, TodoUpdatedPayload.model_validate(todo_state))
...
def _progress_output_risk(sid, name, preview, kw):
    ...
    if isinstance(metadata, dict):
        srv._emit("tool.output_risk", sid, ToolOutputRiskPayload(
...
def _progress_reasoning(sid, name, preview, kw):
    srv._emit("reasoning.available", sid, StreamDeltaPayload(text=str(preview), verbose=True if srv._session_verbose(sid) else None))
...
def _progress_moa_reference(sid, name, preview, kw):
    ...
    srv._emit("moa.reference", sid, MoaReferencePayload.model_validate(ref_payload))
...
def _progress_moa_progress(sid, name, preview, kw):
    ...
    if refs_done is None or refs_total is None:
        return
    srv._emit("moa.progress", sid, MoaProgressPayload(label=str(name or ""), refs_done=int(refs_done), refs_total=int(refs_total)))
...
def _progress_moa_phase(sid, name, preview, kw):
    ...
    srv._emit("moa.phase", sid, MoaPhasePayload.model_validate(phase_payload))
...
_PROGRESS_HANDLERS = {
    ...
    "moa.aggregating": (lambda sid, name, preview, kw: srv._emit("moa.aggregating", sid, MoaAggregatingPayload(aggregator=str(name or ""))), None),
...
def _on_tool_progress(
    ...
    if event_type.startswith("subagent."):
        return srv._progress_subagent(sid, name, preview, _kwargs, event_type)
    if not srv._tool_progress_enabled(sid):
        return
    handler, requires = srv._PROGRESS_HANDLERS.get(event_type, (None, None))
