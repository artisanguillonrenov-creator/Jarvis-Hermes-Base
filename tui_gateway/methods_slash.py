...
def _format_live_review_output(sid: str, session: Optional[dict], arg: str) -> str:
    ...
    tokens = srv._set_session_context(session["session_key"], ui_session_id=sid)
    runtime_token = srv._current_runtime_session_record.set(session)
    try:
...
def _format_live_usage_output(sid: str, session: dict, arg: str) -> str:
    ...
    model = usage.model or srv._metadata_mirror(session).get("model") or getattr(agent, "model", "") or "(unknown)"
...
def _format_live_history_output(sid: str, session: dict, arg: str) -> str:
    ...
    db_history = srv._live_session_messages(session)
    messages = srv._history_to_messages(history if db_history is None else db_history)
    if not messages:
...
def _format_live_prompt_output(sid: str, session: dict, arg: str) -> str:
    agent = session.get("agent")
    mirror = srv._metadata_mirror(session)
    if agent is None and "system_prompt" not in mirror:
        return srv._NO_AGENT
    prompt = (
...
def _format_live_context_output(sid: str, session: dict, arg: str) -> str:
    ...
    try:
        messages = srv._history_to_messages(srv._live_session_messages(session) or [])
    except Exception:
        messages = []  # malformed db rows fall back to the live history below
    if not messages:
        with session["history_lock"]:
            messages = srv._history_to_messages(list(session.get("history", [])))
    usage = srv._session_usage_snapshot(session)
    mirror = srv._metadata_mirror(session)
    ...
    if (agent := session.get("agent")) is not None:
        ...
        tokens = srv._set_session_context(session["session_key"], cwd=srv._session_cwd(session))
        try:
...
def _format_live_tools_output(sid: str, session: dict, arg: str) -> str:
    info = srv._session_info(session.get("agent"), session)
    groups = info.tools
...
def _format_live_status_output(sid: str, session: dict, arg: str) -> str:
    response = srv.invoke("session.status", SessionStatusParams(session_id=sid))
...
def _live_slash_command_output(sid: str, session: Optional[dict], name: str, arg: str) -> Optional[str]:
    ...
    if name == "model" and not arg.strip():
        return srv._format_live_model_output(session or {})
    if name in srv._ISOLATED_SESSION_READ_COMMANDS and not (session is not None and srv._session_uses_compute_host(session)):
        return None
    entry = srv._LIVE_SLASH_OUTPUT.get(name)
    if entry is None:
        return None
...
def _compress_live_with_feedback(sid: str, session: dict, agent, arg: str, *, snapshot_kwargs: bool) -> str:
    ...
    try:
        if snapshot_kwargs:
            srv._compress_session_history(session, arg.strip() or None, **snapshot)
        else:
            ...
            srv._compress_session_history(session, arg)
...
def _mirror_approvals(sid, session, agent, arg) -> None:
    if arg:  # the worker already persisted approvals.mode; the bare read-only form needs no repaint
        srv.broadcast_session_info()


def _mirror_personality(sid, session, agent, arg) -> None:
    if arg and agent:
        pname, new_prompt = srv._validate_personality(arg, srv._load_cfg())
        from hermes_cli.personality import persist_personality  # single owner: no surface drift
        persist_personality(pname)
        srv._apply_personality_to_session(sid, session, new_prompt, pname)


def _mirror_prompt(sid, session, agent, arg) -> None:
    if agent:
        cfg = srv._load_cfg()
        agent.ephemeral_system_prompt = srv._prompt_text((cfg.get("agent") or {}).get("system_prompt", "")) or None
...
def _mirror_fast(sid, session, agent, arg) -> None:
    if agent:
        if arg.lower() in srv._FAST_TIERS:
            agent.service_tier = srv._FAST_TIERS[arg.lower()]
        srv._emit("session.info", sid, SessionInfoPayload.of(srv._session_info(agent, session)))
...
_SLASH_MIRRORS = {
    "model": lambda sid, session, agent, arg: (
        srv._apply_model_switch(sid, session, arg).get("warning", "") if arg and agent else ""),
...
def _compute_host_slash(sid: str, session: dict, name: str, command: str) -> tuple[str, str]:
    ...
    def _on_late_ack(late: dict, _sid=sid) -> None:
        srv._adopt_late_compute_host_compress_ack(_sid, session, late, route_name=route_name)
    try:
        ack = srv._send_compute_host_control(
            sid, route_name=route_name, command=command, wait=True,
            **({"timeout": srv._compute_host_compress_wait_seconds(), "on_late_ack": _on_late_ack} if is_compress else {}))
    except queue.Empty:
        if is_compress:
