...
busy-submit handling. Reaches server.py state through ``srv`` (method_ctx.py)."""
...
def _auto_continue_config() -> tuple[bool, float, int]:
    ...
    desktop = srv._load_cfg().get("desktop")
    cfg = desktop.get("auto_continue") if isinstance(desktop, dict) else None
    cfg = cfg if isinstance(cfg, dict) else {}
    try:
        minutes = float(cfg.get("freshness_minutes", srv._AUTO_CONTINUE_FRESHNESS_MINUTES_DEFAULT))
    except (TypeError, ValueError):
        minutes = float(srv._AUTO_CONTINUE_FRESHNESS_MINUTES_DEFAULT)
    return (is_truthy_value(cfg.get("enabled"), default=True), max(0.0, minutes) * 60.0,
            srv._coerce_int_config_value(cfg.get("max_attempts"), 2, min_value=0))


def _session_home(session: dict) -> Path:
    """The HERMES_HOME the session's durable state lives in (profile-aware)."""
    return Path(session.get("profile_home") or srv._hermes_home)
...
def _retire_turn_marker(session: dict, *keys: str) -> None:
    ...
    for key in dict.fromkeys((*keys, str(session.get("session_key") or ""))):
        if key:
            srv.clear_turn_marker(home, key)
...
def _auto_continue_note(prompt: str) -> str:
    ...
    return (f"{srv._AUTO_CONTINUE_NOTE_PREFIX} — the app or its backend process stopped before the turn could finish. "
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # #94778.
        if srv._ensure_active_session_slot(sid, session) is not None:
        ...
        try:
            srv._emit("status.update", sid, StatusUpdatePayload(kind="process", text="Resuming interrupted turn…"))
            srv._emit("message.start", sid)
            srv._run_prompt_submit(rid, sid, session, text, display_kind="auto_continue")
        except Exception as exc:
            srv._notif_log_failure("auto-continue dispatch failed", exc)
            srv._notif_release_turn(session)  # rebound from session_notifications
...
def _enqueue_prompt(session: dict, text: Any, transport: Any, image_paths: list[str] | None = None,
    ...
    if text_only and not turn_author and text.strip() == srv._ac_inflight_original(session) != "":
        return
...
def _drop_queued_duplicates_of_inflight_user(session: dict) -> None:
    ...
    """
    if not (original := srv._ac_inflight_original(session)):
        return
    head = session.get("queued_prompt")
    cleaned = (srv._sanitize_queued_entry_vs_inflight_user(e, original)
               for e in ([head] if head else []) + list(session.get("queued_prompts") or []))
    srv._ac_set_queue(session, [c for c in cleaned if c is not None])
...
def _interrupt_busy_session(sid: str, session: dict, agent: Any) -> None:
    ...
    if not use_agent and not srv._session_uses_compute_host(session):
        return
    with session["history_lock"]:
    ...
    def interrupt() -> None:
        try:
            with contextlib.suppress(Exception):
                agent.interrupt() if use_agent else srv._get_compute_host_supervisor().interrupt(sid)
        finally:
            with session["history_lock"]:
...
def _ac_try_correction(rid, session: dict, agent: Any, method: str, plain_text: str,
    ...
    try:
        ...
        return None
    with session["history_lock"]:
        srv._record_inflight_correction(session, plain_text)
        srv._drop_queued_duplicates_of_inflight_user(session)
        session["last_active"] = time.time()
...
def _handle_busy_submit(rid, sid: str, session: dict, text: Any, transport: Any, queued: bool = False,
    ...
    mode = "queue" if queued else srv._load_busy_input_mode()
    ...
    plain_text = srv._coerce_message_text(text).strip() if not image_paths and srv._is_text_only_busy_payload(text) else ""
    ...
    if plain_text and agent is not None:
        ...
        if (method and supported[mode]
                and (resp := srv._ac_try_correction(rid, session, agent, method, plain_text, status)) is not None):
            return resp
    ...
    with session["history_lock"]:
        if not session.get("running"):
            ...
            return None
        srv._enqueue_prompt(session, text, transport, image_paths=image_paths, turn_author=turn_author)
        session["last_active"] = time.time()
    ...
    if mode == "interrupt" and not image_paths:
        srv._interrupt_busy_session(sid, session, agent)
...
def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    ...
    with srv._session_turn_admission(session) as admitted:
        ...
        srv._ac_set_queue(session, session.get("queued_prompts") or [])
        session["running"] = True
        ...
        if queued_transport is not None and not srv._transport_is_dead(queued_transport):
            srv._attach_session_transport(session, queued_transport)
    ...
    with session["history_lock"]:
        if int(session.get("_queued_prompt_generation", 0)) != queue_generation:
            ...
            srv._ac_set_queue(session, [queued, *([advanced] if advanced else []), *(session.get("queued_prompts") or [])])
            session["running"] = False
            return True
    ...
    try:
        if not use_compute_host:
            srv._run_prompt_submit(rid, sid, session, queued["text"], **kwargs, **author_kwargs)
        elif isinstance(resp := srv._submit_prompt_to_compute_host(rid, sid, session, queued["text"], **kwargs), dict):
            with session["history_lock"]:
                session["running"] = False
                srv._clear_inflight_turn(session)
            srv._emit("error", sid, ErrorPayload(message=str((resp.get("error") or {}).get("message") or "queued prompt failed")))
            dispatch_failed = True
    except Exception as exc:
        srv._notif_log_failure("queued prompt dispatch failed", exc)
...
def _emit_terminal_turn_error(
    ...
    with session["history_lock"]:
        srv._fail_inflight_turn(session, error, error_surface=error_surface)
    ...
    text = partial or srv.turn_error_text(message, error_surface)
    rendered = ""
    with contextlib.suppress(Exception):
        rendered = srv.render_message(text, cols)
    payload = {"text": text, "usage": srv._get_usage(agent) if agent is not None else {}, "status": "error",
    ...
    if retire_marker:
        srv._retire_turn_marker(session)
...
def _queued_prompt_snapshot(session: dict) -> dict | None:
    ...
    user = srv._inflight_text(queued.get("text")) if isinstance(queued, dict) else ""
