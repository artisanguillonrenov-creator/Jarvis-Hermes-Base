...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # Approval delegation: an admin may resolve approvals belonging to *other*
        # users' sessions, so this check must run BEFORE the local pending-approval
        # gate below (which would answer "nothing pending" in the admin's own chat).
    ...
    async def _handle_deny_command(self, event: MessageEvent) -> str:
        ...
        """
        # Approval delegation: as in /approve, must run BEFORE the local
        # pending-approval gate (the admin resolves *other* sessions).
