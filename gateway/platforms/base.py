...
# attachment is silently dropped (#88038). Some providers can also leak their exact terminal
# ``<|eos|>`` control token directly after a valid path. Treat that one token as part of the
# directive only when it terminates the whole response; accepting ``<`` generally would turn
# arbitrary adjacent markup into a path delimiter (#111046).
