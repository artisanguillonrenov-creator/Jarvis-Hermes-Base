...
describe('renderMediaTags', () => {
  ...
  it('keeps a following ordinary file URL outside the MEDIA path', () => {
    const source = 'MEDIA:/srv/absolute.txt https://example.com/report.txt'

    expect(renderMediaTags(source)).toBe(
      '[File: absolute.txt](#media:%2Fsrv%2Fabsolute.txt) https://example.com/report.txt'
    )
    expect(mediaPathsFromText(source)).toEqual(['/srv/absolute.txt'])
  })
  ...
  it('restores a provisional MEDIA source when a resumed stream appends the rest of the path', () => {
    let parts = [assistantTextPart('MEDIA:/tmp/final', 1)]

    expect(parts[0]).toHaveProperty('provisionalMediaSource', 'MEDIA:/tmp/final')
    parts = appendAssistantTextPart(parts, ' report.pdf\n', 2)

    expect(chatMessageText({ id: 'a', role: 'assistant', parts })).toBe(
      '[File: final report.pdf](#media:%2Ftmp%2Ffinal%20report.pdf)\n'
    )
    expect(parts[0]).not.toHaveProperty('provisionalMediaSource')
  })
