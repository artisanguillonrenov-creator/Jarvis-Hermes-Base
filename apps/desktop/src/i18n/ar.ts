...
export const ar = defineLocale({
  ...
  statusStack: {
    ...
    coding: {
      ...
      restack: 'إعادة ترتيب الإيداعات',
      restackTip: 'اطلب من Hermes إعادة تنظيم هذا الفرع في مجموعة إيداعات أوضح (نفس الملفات، سجل أوضح)',
      restackPending: 'Hermes يعيد ترتيب الإيداعات…',
      restackNothing: 'لا شيء لإعادة ترتيبه: لا يملك هذا الفرع إيداعات خاصة به وشجرة العمل نظيفة.',
      restackPromptIntro:
        'أعد ترتيب هذا الفرع: أعد كتابة الإيداعات منذ قاعدة الدمج {base} إلى مجموعة نظيفة ومنطقية الترتيب للمراجعة. المجموعة الحالية:',
      restackPromptDirty: 'توجد أيضًا تغييرات غير مودعة في شجرة العمل؛ أدرجها في الإيداعات المناسبة.',
      restackPromptRules:
        'القواعد: يجب أن تكون الشجرة النهائية مطابقة بايتًا لبايت لشجرة العمل الحالية (أنشئ مرجع لقطة أولًا، ثم تحقق بـ `git diff` مقابله في النهاية وأبلغ بالنتيجة). افصل التغييرات غير المترابطة في إيداعات منفصلة، وادمج التصحيحات والأعمال المؤقتة في الإيداع الذي تنتمي إليه، وأعطِ كل إيداع رسالة واضحة بأسلوب الإيداعات التقليدية مع متن يوضح السبب. لا تدفع، ولا تلمس فروعًا أخرى، وتوقف واسأل قبل حل أي تعارض يغيّر محتوى الملفات.',
      commitPicker: 'الإيداع المطلوب مراجعته',
      commitPickerWorkingTree: 'شجرة العمل',
      commitFiles: count => (count === 1 ? 'ملف واحد' : `${count} ملفات`)
    }
  },
  updates: {
    stages: {
