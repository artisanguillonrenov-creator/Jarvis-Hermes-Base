...
export const zh = defineLocale({
  ...
  settings: {
    ...
    appearance: {
      ...
      turnPerfTitle: '显示令牌速率',
      turnPerfDesc: '在已完成的助手回复（含多步骤智能体执行）上显示所用令牌数与每秒令牌数。',
      embedsTitle: '内嵌预览',
      embedsDesc:
  ...
  assistant: {
    thread: {
      ...
      turnPerfCount: tokens => `${tokens} 令牌`,
      turnPerfRate: rate => `${rate} tok/s`,
      turnPerfTooltipDuration: duration => `本轮耗时 ${duration}`,
      turnPerfTooltipTokens: tokens => `${tokens} 输出令牌（供应商实测）`,
      turnPerfTooltipTokensEst: tokens => `≈${tokens} 输出令牌（估算）`,
      turnPerfTooltipThinking: duration => `思考用时 ${duration}`,
      today: time => `今天，${time}`,
