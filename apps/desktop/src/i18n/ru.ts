...
export const ru = defineLocale({
  ...
  statusStack: {
    ...
    coding: {
      ...
      worktrees: 'Worktrees',
      restack: 'Перестроить коммиты',
      restackTip: 'Попросить Hermes переупорядочить ветку в более чистый набор коммитов (те же файлы, понятнее история)',
      restackPending: 'Hermes перестраивает коммиты…',
      restackNothing: 'Нечего перестраивать: у ветки нет собственных коммитов, а рабочее дерево чистое.',
      restackPromptIntro:
        'Перестрой эту ветку: перепиши коммиты после merge base {base} в чистый, логически упорядоченный набор для ревью. Текущий стек:',
      restackPromptDirty: 'В рабочем дереве также есть незакоммиченные изменения; включи их в подходящие коммиты.',
      restackPromptRules:
        'Правила: итоговое дерево должно побайтно совпадать с текущим рабочим деревом (сначала создай snapshot-ref, в конце проверь через `git diff` относительно него и сообщи результат). Разделяй несвязанные изменения на отдельные коммиты, сливай fixup и промежуточный WIP в коммиты, к которым они относятся, и давай каждому коммиту понятное сообщение в стиле conventional commits с телом, объясняющим зачем. Не пушь, не трогай другие ветки и остановись и спроси перед разрешением любого конфликта, который изменит содержимое файлов.',
      commitPicker: 'Коммит для ревью',
      commitPickerWorkingTree: 'Рабочее дерево',
      commitFiles: count => `${count} ${RU_NOUN(count, 'файл', 'файла', 'файлов')}`
    }
  },
  updates: {
    stages: {
