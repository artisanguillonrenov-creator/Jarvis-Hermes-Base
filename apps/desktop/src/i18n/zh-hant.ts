...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    appearance: {
      ...
      turnPerfTitle: '顯示令牌速率',
      turnPerfDesc: '在已完成的助手回覆（含多步驟智能體執行）上顯示所用令牌數與每秒令牌數。',
      embedsTitle: '內嵌預覽',
      embedsDesc:
  ...
  assistant: {
    thread: {
      ...
      turnPerfTooltipDuration: duration => `本輪耗時 ${duration}`,
      turnPerfTooltipTokens: tokens => `${tokens} 輸出令牌（供應商實測）`,
      turnPerfTooltipTokensEst: tokens => `≈${tokens} 輸出令牌（估算）`,
      turnPerfTooltipThinking: duration => `思考用時 ${duration}`,
      today: time => `今天，${time}`,
