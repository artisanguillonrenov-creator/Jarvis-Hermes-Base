...
it('tracks only the owning runtime session, independent of foreground busy state and child stream text', () => {
  ...
  upsertSubagent('owner', subagentEvent({ subagent_id: 'working', status: 'running' }), true, 'subagent.start')
  upsertSubagent('owner', subagentEvent({ subagent_id: 'waiting', status: 'queued' }), true, 'subagent.spawn_requested')
  upsertSubagent('other', subagentEvent({ subagent_id: 'working', status: 'running' }), true, 'subagent.start')
  $activeSessionId.set('other')
  $busy.set(true)
  upsertSubagent('owner', subagentEvent({ subagent_id: 'working', text: '(°□°) pondering...' }), false, 'subagent.thinking')
  expect(owner.get()).toEqual({ count: 2 })
  expect(other.get()).toEqual({ count: 1 })
  expect(unbound.get()).toBeNull()

  upsertSubagent('owner', subagentEvent({ subagent_id: 'working', status: 'completed' }), false, 'subagent.complete')
  expect(owner.get()).toEqual({ count: 1 })
  upsertSubagent('owner', subagentEvent({ subagent_id: 'waiting', status: 'timeout' }), false, 'subagent.complete')
  $activeSessionId.set(null)
  $busy.set(false)
