    ...
    async (selection: ModelSelection): Promise<boolean> => {
      ...
      try {
        ...
        if (result?.confirm_required) {
          ...
          // and that answer only exists after the user answers the dialog.
          // Declining is free — nothing has been applied at this point.
          void surfaceModelSwitchConfirm({
            confirmMessage: result.confirm_message,
            failureMessage: copy.modelSwitchFailed,
            finish: finishSwitch,
            // Staleness guard — the session or the model can move on while
            // the dialog is open. Answering it must not clobber the newer
            // choice: bail (with a notice) if the live state no longer
            // matches the snapshot this prompt was created for.
            isStale: () =>
              touchesPrimary
