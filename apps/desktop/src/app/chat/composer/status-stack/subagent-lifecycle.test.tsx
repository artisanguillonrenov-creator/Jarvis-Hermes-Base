...
it('keeps each worker draft while inspecting siblings and removes settled selection', () => {
  upsertSubagent('parent', subagentEvent({ subagent_id: 'a', goal: 'Worker A' }))
  upsertSubagent('parent', subagentEvent({ subagent_id: 'b', goal: 'Worker B' }))
  ...
  act(() =>
    upsertSubagent('parent', subagentEvent({ subagent_id: 'a', status: 'completed' }), false, 'subagent.complete')
  )
...
it('measures detail elapsed from worker start rather than first inspection', () => {
  ...
  upsertSubagent('parent', subagentEvent({ subagent_id: 'timed', goal: 'Timed worker' }))
