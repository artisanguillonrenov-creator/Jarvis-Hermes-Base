...
// German bundle. Mirrors the full shape of `en`: plugin catalogs have no
// per-key fallback, so every key must be present.
const de: BotsMessages = {
  roster: {
    search: 'Bots und Gruppenchats suchen',
    searchPlaceholder: 'Bots und Gruppenchats suchen…',
    newBotOrGroup: 'Neuer Bot oder Gruppenchat',
    groupChats: 'Gruppenchats',
    emptyTitle: 'Noch keine Bots',
    emptyDesc: 'Lege deinen ersten Bot an.',
    ...
    waitingForGateway:
      'Warte auf die Gateway-Verbindung… (Remote-Gateways können ein paar Sekunden brauchen; wird automatisch wiederholt)'
  },
  sections: {
    ...
    options: name => `Optionen für Bereich ${name}`,
    ...
    removeFromSection: 'Aus Bereich entfernen',
    deleted: (name, count) =>
      count === 0
        ? `„${name}“ gelöscht`
  ...
  bot: {
    ...
    descriptionHint: 'Leer lassen, um aus Name und Beschreibung des Bots zu generieren.',
    newChatWith: 'Neuer Chat mit diesem Bot',
    ...
    createFailed: 'Profil konnte noch nicht angelegt werden',
    advanced: 'Erweitert',
    ...
    remoteConnectionsUnsupported:
      'Aktualisiere Hermes Desktop, um mit Bots auf anderen Verbindungen zu chatten.',
    chatEmpty: 'Schreib etwas, um loszulegen.',
  ...
  avatar: {
    ...
    describeHint:
      'Leer lassen, um automatisch aus Name/Titel/Beschreibung + Agent-Messaging-Liste zu generieren.',
    matchTheName: 'Zum Namen passend',
    pickPet: 'Wähle ein Tier als Profilbild dieses Bots.',
    petLoadFailed: 'Dieses Tier konnte nicht geladen werden — probier ein anderes.',
    imageTooLarge: 'Bild zu groß (max. 15 MB).',
    generationFailed: 'Avatar-Generierung fehlgeschlagen',
    savedLocally: 'Aussehen lokal gespeichert; Speichern auf dem Server fehlgeschlagen',
    savedLocallyDescriptionFailed: 'Aussehen lokal gespeichert; Beschreibung konnte nicht aktualisiert werden',
    generate: 'Generieren',
    generating: 'Wird generiert…'
  },
  group: {
    newTitle: 'Neuer Gruppenchat',
    manageDesc: 'Ein Bot kann in mehreren Gruppenchats sein. Mitgliedschaften werden auf alle Geräte synchronisiert.',
    manageTitle: 'Gruppen verwalten',
    settingsTitle: 'Gruppeneinstellungen',
    settingsDesc: 'Benenne die Gruppe um oder setze ein Raumbild. Mitglieder und Verlauf bleiben erhalten.',
    ...
    slashCommandsUnsupported:
      'Slash-Befehle funktionieren in Gruppenchats nicht. Öffne einen einzelnen Bot-Chat, um sie zu nutzen.',
    ...
    activity: 'Aktivität',
    noActivityYet: 'In dieser Runde noch keine Aktivität.',
    showActivity: 'Raumaktivität anzeigen',
    hideActivity: 'Raumaktivität ausblenden',
    stop: 'Stoppen',
    stopHint: 'Diesen Lauf stoppen — unterbricht das Mitglied am Zug und hält den Rest an',
    allHeldStatus: count => `Alle ${count} Bots sind pausiert`,
    heldMembersStatus: members => `Pausiert: ${members}`,
    holdReleaseHint: 'Erwähne einen pausierten Bot oder sende @all resume, um sie fortzusetzen.',
    needsYourInput: 'Ein Bot in diesem Gruppenchat braucht deine Eingabe',
    pictureGenerationFailed: 'Generierung des Gruppenbilds fehlgeschlagen',
    nameTaken: name => `Eine Gruppe namens „${name}“ existiert bereits.`,
    memberCount: count => `${count} Bots`,
    settingsHint: group => `Gruppeneinstellungen — ${group} umbenennen oder Raumbild setzen`,
    ...
    disbandDescSuffix: count =>
      ` von ihren ${count} Bots und löscht das gemeinsame Raum-Log. Die Bots selbst und ihre Sessions pro Gruppe bleiben erhalten.`,
    stopped: group => `${group} gestoppt — die restlichen Züge warten, bis du fortsetzt`,
    removeAttachment: 'Anhang entfernen',
    threadFallback: 'Thread',
    replyCount: replies => `${replies} ${replies === 1 ? 'Antwort' : 'Antworten'}`,
    dropToThread: 'Hier ablegen, um an diese Thread-Antwort anzuhängen',
    dropToRoom: 'Hier ablegen zum Anhängen — jeder antwortende Bot sieht es',
    ...
    newThreadPlaceholder: group => `Neuer Thread in ${group}… (@name für gezielt, @everyone für alle)`,
    everyoneMeta: 'Jeder Bot im Raum',
    commandApproval: 'Befehlsfreigabe',
    answerFailed: (handle, error) => `Antwort an @${handle} konnte nicht gesendet werden: ${error}`,
    wantsToRunCommand: handle => `@${handle} will einen Befehl ausführen:`,
  ...
  tools: {
    ...
    searchHub: 'Hub durchsuchen (Community + bekannte Quellen)…',
    noMcpServers: 'Keine MCP-Server konfiguriert oder im Katalog.'
  },
  cron: {
    filterHint:
      'In diesem Profil gibt es geplante Aufgaben, aber keine ist für diesen Bot markiert. Benenne eine Aufgabe "[bot:<name>] …", damit sie hier erscheint, oder sieh sie unten unter Cron.',
    needsRosterFirst: 'Dieser Bot muss zuerst in der Liste erscheinen.',
    staleNotice: 'Geplante Aufgaben konnten nicht aktualisiert werden. Angezeigt wird die zuletzt bekannte Liste.',
    readFailure: 'Die Liste ist womöglich noch da — das war ein Lesefehler, kein Löschen.',
    createDesc: bot => `Eine wiederkehrende Aufgabe, die ${bot} nach Zeitplan ausführt. Läufe landen im eigenen Chatverlauf.`,
    instruction: 'Anweisung',
    whenToRun: 'Wann ausführen',
    dayOfMonth: 'Tag im Monat',
    sendResultsTo: 'Ergebnisse senden an',
    runHistoryOnly: 'Nur Lauf-Historie',
    botChatTarget: bot => `Chat von ${bot} (Bot antwortet)`,
    continuity: 'Kontinuität: Jeder Lauf sieht die Ausgabe des vorherigen Laufs (Dubletten vermeiden, dort weitermachen, wo er aufgehört hat)',
    onceIn: when => `Einmal (${when})`,
    ...
    runsWeekdays: time => `Läuft Montag–Freitag um ${time}`,
    runsWeekly: (day, time) => `Läuft jeden ${day} um ${time}`,
    runsMonthly: (day, time) => `Läuft am ${day}. Tag jedes Monats um ${time}`,
    runsInterval: (count, unit) => `Läuft alle ${count} ${unit}`,
    runsRaw: 'Roher Zeitplan — every Nm/Nh/Nd oder 5-field cron',
...
export const BOTS_LOCALES: PluginLocaleBundles = { en, ja, zh, 'zh-hant': zhHant, de }
