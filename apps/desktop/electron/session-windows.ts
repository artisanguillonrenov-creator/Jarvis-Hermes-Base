...
function buildInstanceWindowUrl({ connectionId, devServer, profile, rendererIndexPath }: any = {}) {
  const query = `?peer=1${profile ? `&profile=${encodeURIComponent(profile)}&connectionId=${encodeURIComponent(connectionId ?? '')}` : ''}`
