 ...
 *
 * Config GET responses are stamped with the effective route Electron actually
 * dispatched to (`CONFIG_SERVED_ROUTE_KEY`) so the renderer can bind PUT to
 * that backend — not the ambient request scope that may have omitted the id.
 */
async function dispatchApiRequestForSelectedConnection<T>(
  ...
  const isConfigPath = isConfigApiPath(pathname)

  const remotePrimaryConnectionId = isConfigPath
    ? String(options.remotePrimaryConnectionId || '').trim()
    : ''

  const requestProfile = String(request?.profile ?? '').trim() || undefined

  let result: T
  let servedConnectionId: string | undefined

  if (explicitConnectionId) {
    result = await deps.dispatchRegistry(request, explicitConnectionId)
    servedConnectionId = explicitConnectionId
  } else if (isConfigPath && isConfigWriteMethod(request?.method)) {
    ...
    result = await deps.dispatchProfile(request)
    servedConnectionId = undefined
  } else {
    const connectionId = remotePrimaryConnectionId

    if (connectionId) {
      result = await deps.dispatchRegistry(request, connectionId)
      servedConnectionId = connectionId
    } else {
      result = await deps.dispatchProfile(request)
      servedConnectionId = undefined
    }
  ...
  if (isConfigPath && !isConfigWriteMethod(request?.method) && result && typeof result === 'object') {
    stampConfigServedRoute(result, {
      ...(servedConnectionId ? { connectionId: servedConnectionId } : {}),
...
/** Must match `CONFIG_SERVED_ROUTE_KEY` in apps/desktop/src/api/config.ts. */
const CONFIG_SERVED_ROUTE_KEY = '__hermesConfigServedRoute'

function stampConfigServedRoute(
  payload: object,
  route: { connectionId?: string; profile?: string }
): void {
  Object.defineProperty(payload, CONFIG_SERVED_ROUTE_KEY, {
    configurable: true,
    enumerable: true,
    value: {
